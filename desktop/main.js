...
/**
 * The app's own mark, for the window and the tray.
 *
 * One picture, made by `build/icon.mjs` from the same shape the page draws at
 * the top of the rail. Two marks that are nearly the same is worse than one:
 * somebody looking for this in a taskbar is looking for a thing they have
 * already seen, and a second version of it is a second thing to learn.
 *
 * Missing is survivable. It is generated by `npm run icon` and by the build,
 * and somebody running from source who has done neither gets the default
 * rather than a window that will not open.
 */
function theMark() {
  const at = join(__dirname, '..', 'build', 'icon.png');
  if (!existsSync(at)) return null;
  const mark = nativeImage.createFromPath(at);
...
function sitInTheTray() {
  ...
  // The same mark, small. A tray icon that is a different picture from the
  // window's is two marks for one app.
  const dot = theMark()?.resize({ width: 32, height: 32 }) ?? nativeImage.createFromDataURL(
    'data:image/svg+xml;base64,' + Buffer.from(
      `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32">
         <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1">
           <stop offset="0" stop-color="#7c5cfc"/><stop offset="1" stop-color="#9b7dff"/>
         </linearGradient></defs>
