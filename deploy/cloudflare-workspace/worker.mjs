/**
 * Viberant's free Cloudflare Internet Workspace edge.
 *
 * The WorkspacePlane Durable Object keeps only capability hashes, short-lived
 * presence, invitation fingerprints and one-use relay tickets. RelayPair sees
 * encrypted binary messages only, forwards them immediately, and uses the
 * hibernation API while idle. No project, filename, chat text, source, key or
 * build output is stored here.
 */

const PLANE_PATHS = new Set([
  'announce', 'who-is-about', 'ticket', 'wait-for-ticket', 'forget',
  'offer-invite', 'find-invite', 'join-ticket',
]);
const STILL_HERE = 90_000;
const NOT_TOO_OLD = 120_000;
const TICKET_LASTS = 60_000;
const MOST_BODY = 256 * 1024;
const MOST_RELAY_MESSAGE = 9 * 1024 * 1024;
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && PLANE_PATHS.has(path)) {
      ...
      const local = ['127.0.0.1', 'localhost', '::1'].includes(relay.hostname);
      relay.protocol = relay.protocol === 'http:' && local ? 'ws:' : 'wss:';
...
export class WorkspacePlane {
  ...
  async relayTicket(workspace, from, to) {
    ...
    await this.ctx.storage.put(`relay:${ticket}`, {
      workspace, from, to, expiresAt: Date.now() + TICKET_LASTS,
    });
    return ticket;
  }
  ...
  async takePending(workspace, deviceId) {
    ...
    if (live.length) await this.ctx.storage.put(key, live); else await this.ctx.storage.delete(key);
    return first;
  }
  ...
  async fetch(request) {
    ...
    if (internal) {
      let ticket;
      try { ticket = String((await request.json()).ticket ?? ''); } catch { return json({ ok: false }, 400); }
    ...
    if (path === 'join-ticket') {
      ...
      // One public use. The owner's membership book remains the final check.
      await this.ctx.storage.delete(`invite:${mark}`);
      ...
      await this.deliver(invitation.workspace, invitation.owner.deviceId, {
        kind: 'join', ticket, workspace: invitation.workspace, mark,
        from: from.deviceId, card: from, to: invitation.owner.deviceId,
        relay: invitation.relay, at: Date.now(),
      });
    ...
    if (path === 'ticket') {
      ...
      await this.deliver(workspace, String(body.to), {
        ticket, workspace, from: from.deviceId, to: String(body.to), relay, at: Date.now(),
      });
    ...
    if (path === 'wait-for-ticket') {
      ...
      const waiting = new Promise((done) => {
        ...
        if (old) { clearTimeout(old.timer); old.done(null); }
        const timer = setTimeout(() => {
          settleWaiting(null);
        }, 20_000);
        this.waiters.set(key, { done: settleWaiting, finish: settleWaiting, timer });
      });
      // A ticket can land after the first read and before the waiter is
      // installed. Recheck once so that race costs no twenty-second delay.
    ...
    if (path === 'offer-invite') {
      ...
      await this.ctx.storage.put(`invite:${body.mark}`, {
        workspace, mark: body.mark, expiresAt, owner: from,
...
export class RelayPair {
  ...
  webSocketMessage(socket, message) {
    ...
    try { other.send(message); } catch { socket.close(1011, 'relay interrupted'); }
  }

  webSocketClose(socket, code, reason) {
    const other = this.ctx.getWebSockets().find((one) => one !== socket);
    try { other?.close(code || 1000, String(reason || 'other computer left').slice(0, 120)); } catch { /* gone */ }
  }

  webSocketError(socket) {
    const other = this.ctx.getWebSockets().find((one) => one !== socket);
    try { other?.close(1011, 'relay interrupted'); } catch { /* gone */ }
  }
}

export const __testOnly = { signedRequest, hashText, PLANE_PATHS };
